...
if [[ -v VERSION ]]; then
    ...
    docker build -t eventlistener/eventlistener:${VERSION} -f server/Dockerfile server/.
    docker build -t eventlistener/eventlistener:${TYPE} -f server/Dockerfile server/.
    docker push eventlistener/eventlistener:${VERSION}
    docker push eventlistener/eventlistener:${TYPE}
fi