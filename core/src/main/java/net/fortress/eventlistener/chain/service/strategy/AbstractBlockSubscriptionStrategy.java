...
package net.fortress.eventlistener.chain.service.strategy;

import io.reactivex.disposables.Disposable;
import lombok.extern.slf4j.Slf4j;
import net.fortress.eventlistener.chain.block.BlockListener;
import net.fortress.eventlistener.chain.service.block.BlockNumberService;
import net.fortress.eventlistener.chain.service.domain.Block;
import net.fortress.eventlistener.chain.service.domain.wrapper.Web3jBlock;
import net.fortress.eventlistener.service.AsyncTaskService;
import net.fortress.eventlistener.utils.ExecutorNameFactory;
...
public abstract class AbstractBlockSubscriptionStrategy<T> implements BlockSubscriptionStrategy {
    ...
    protected void triggerListeners(T blockObject) {
        final Block eventlistenerBlock = convertToEventlistenerBlock(blockObject);

        if (eventlistenerBlock != null) {
            triggerListeners(eventlistenerBlock);
        }
    }

    protected void triggerListeners(Block eventlistenerBlock) {
        asyncService.execute(ExecutorNameFactory.build(BLOCK_EXECUTOR_NAME, eventlistenerBlock.getNodeName()), () -> {
            ...
            if (lastBlockNumberProcessed.get() > 0 && eventlistenerBlock.getNumber().compareTo(expectedBlock) > 0) {

                final int missingBlocks = eventlistenerBlock.getNumber()
                        .subtract(expectedBlock)
                        .intValue();

                log.warn("Missing {} blocks.  Expected {}, got {}.  Catching up...",
                        missingBlocks, expectedBlock, eventlistenerBlock.getNumber());
            ...
            blockListeners.forEach(listener -> triggerListener(listener, eventlistenerBlock));
            updateLastBlockProcessed(eventlistenerBlock);
        });
    }
    ...
    abstract Block convertToEventlistenerBlock(T blockObject);
