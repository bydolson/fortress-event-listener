# Eventlistener
An Ethereum event listener that bridges your smart contract events and backend microservices. Eventlistener listens for specified event emissions from the Ethereum network, and broadcasts these events into your middleware layer. This provides a distinct separation of concerns and means that your microservices do not have to subscribe to events directly to an Ethereum node.

[![Gitter](https://badges.gitter.im/eventlistener/community.svg)](https://gitter.im/eventlistener/community?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)

**Master**

[![CircleCI](https://circleci.com/gh/ConsenSys/eventlistener/tree/master.svg?style=svg)](https://circleci.com/gh/ConsenSys/eventlistener/tree/master)

**Development**

[![CircleCI](https://circleci.com/gh/ConsenSys/eventlistener/tree/development.svg?style=svg)](https://circleci.com/gh/ConsenSys/eventlistener/tree/development)

## Features
* Dynamically Configurable - Eventlistener exposes a REST api so that smart contract events can be dynamically subscribed / unsubscribed.

* Highly Available - Eventlistener instances communicate with each other to ensure that every instance is subscribed to the same collection of smart contract events.
...
* Fork Tolerance - Eventlistener can be configured to wait a certain amount of blocks before an event is considered 'Confirmed'.  If a fork occurs during this time, a message is broadcast to the network, allowing your services to react to the forked/removed event.
...
## Eventlistener Tutorials
- [Listening to Ethereum Events](https://kauri.io/article/90dc8d911f1c43008c7d0dfa20bde298/listening-to-ethereum-events-with-eventlistener)
- [Listening for Ethereum Transactions](https://kauri.io/article/3e31587c96a74d24b5cdd17952d983e9/v1/listening-for-ethereum-transactions-with-eventlistener)
- [Using Eventlistener to Build a Java Smart Contract Data Cache](https://kauri.io/article/fe81ee9612eb4e5a9ab72790ef24283d/using-eventlistener-to-build-a-java-smart-contract-data-cache)

## Getting Started
Follow the instructions below in order to run Eventlistener on your local machine for development and testing purposes.
...
$ java -jar target/eventlistener-server.jar
```
...
```sh
$ cd server
$ docker build  . -t kauri/eventlistener:latest
...
$ docker run -p 8060:8060 kauri/eventlistener
```
...
When upgrading Eventlistener to **0.8.0**, changes in the schema are required. In order to perform the migration follow these steps:
...
4. Restart Eventlistener instances


## Configuring Nodes
Listening for events from multiple different nodes is supported in Eventlistener, and these nodes can be configured in the properties file.
...
### REST
Eventlistener exposes a REST api that can be used to register events that should be subscribed to / broadcast.
...
Static events can be configured within the application.yml file of Eventlistener.
...
From version 0.6.2, eventlistener supports monitoring and broadcasting transactions. The matching criteria can be:
...
## Configuration
Eventlistener can either be configured by:
...
| -------- | -------- | -------- |
| SERVER_PORT | 8060 | The port for the eventlistener instance. |
...
| ETHEREUM_NODE_ADD_TRANSACTION_REVERT_REASON | false | In case of a failing transaction it indicates if Eventlistener should get the revert reason. Currently not working for Ganache and Parity.
...
| EVENTSTORE_TYPE | DB | The type of eventstore used in Eventlistener. (See the Advanced section for more details) |
| BROADCASTER_TYPE | KAFKA | The broadcast mechanism to use.  (KAFKA or HTTP or RABBIT) |
| BROADCASTER_CACHE_EXPIRATIONMILLIS | 6000000 | The eventlistener broadcaster has an internal cache of sent messages, which ensures that duplicate messages are not broadcast.  This is the time that a message should live within this cache. |
...
| BROADCASTER_MULTIINSTANCE | false | If multiple instances of eventlistener are to be deployed in your system, this should be set to true so that the eventlistener communicates added/removed filters to other instances, via kafka. |
...
| SYNCINC_THRESHOLD | 60 | Number of blocks of difference to consider that eventlistener is "syncing" with a node
...
Eventlistener utilises an event store in order to establish the block number to start event subscriptions from, in the event of a failover.  For example, if the last event broadcast for event with id X had a block number of 123, then on a failover, eventlistener will subscribe to events from block 124.
...
### Integrating Eventlistener into Third Party Spring Application

Eventlistener can be embedded into an existing Spring Application via an annotation.

#### Steps to Embed

1. Add the Fortress Kauri bintray repository into your `pom.xml` file:

```xml
<repositories>
  <repository>
    <id>bintray-fortress-kauri</id>
    <url>https://fortress.bintray.com/kauri</url>
  </repository>
</repositories>
```

2. Add the eventlistener-core dependency to your `pom.xml` file:

```xml
<dependency>
  <groupId>net.fortress.eventlistener</groupId>
  <artifactId>eventlistener-core</artifactId>
  <version>*LATEST_EVENTLISTENER_VERSION*</version>
</dependency>
```

3. Within your Application class or a `@Configuration` annotated class, add the `@EnableEventlistener` annotation.

#### Health check endpoint

Eventlistener offers a healthcheck url where you can ask for the status of the systems you are using. It will look like:
...
Eventlistener includes a prometheus metrics export endpoint.
...
* eventlistener_%Network%_syncing: 1 if node is syncing (latestBlock + syncingThreshols < currentBlock). 0 if not syncing
* eventlistener_%Network%_latestBlock: latest block read by Eventlistener
* eventlistener_%Network%_currentBlock: Current node block
* eventlistener_%Network%_status: Current node status. 0 = Suscribed, 1 = Connected, 2 = Down

All  metrics include application="Eventlistener",environment="local" tags.

The endpoint is: GET /monitoring/prometheus


## Known Caveats / Issues
* In multi-instance mode, where there is more than one Eventlistener instance in a system, your services are required to handle duplicate messages gracefully, as each instance will broadcast the same events.
